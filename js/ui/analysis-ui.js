...
import { getStockById, getStocksByPortfolio, fetchKlineCached, updateStock, fetchStockNews } from '../api.js';
import { analyze, analyzeEntry, analyzeAdd } from '../analysis.js';
...
// ── In-page state (stock switch + add-position tab) ──
let currentStock = null;
let currentKlines = null;
let currentSiblings = [];
let currentMode = 'manage'; // 'manage' | 'add'

function switcherHtml(currentId) {
    if (!currentSiblings || currentSiblings.length <= 1) return '';
    const items = currentSiblings.map(s => {
        const active = s.id === currentId ? ' active' : '';
        const check = s.id === currentId ? ' <i class="bi bi-check2"></i>' : '';
        const stLabel = s.status === 'watching' ? '关注' : s.status === 'sold' ? '已卖出' : '持有';
        const stBadge = s.status === 'watching' ? 'bg-info' : s.status === 'sold' ? 'bg-secondary' : 'bg-success';
        return `<li><a class="dropdown-item d-flex justify-content-between align-items-center${active}" href="analysis?id=${s.id}">
            <span><span class="badge bg-light text-dark me-1">${s.code}</span>${escapeHtml(s.name)} <span class="badge ${stBadge}">${stLabel}</span></span>${check}
        </a></li>`;
    }).join('');
...
function tabsHtml(active) {
    return `<ul class="nav nav-pills nav-sm mb-3" id="analysis-tabs">
        <li class="nav-item"><a class="nav-link ${active === 'manage' ? 'active' : ''}" data-mode="manage" href="#">持仓管理</a></li>
        <li class="nav-item"><a class="nav-link ${active === 'add' ? 'active' : ''}" data-mode="add" href="#">加仓分析</a></li>
    </ul>`;
}

function headerHtml(stock, badgesHtml, activeTab) {
    return `<div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        <div class="d-flex align-items-center gap-2 flex-wrap">
            ${switcherHtml(stock.id)}
            <h4 class="mb-0">
    ...
    ${activeTab ? tabsHtml(activeTab) : ''}`;
}

function bindTabs() {
    const tabs = document.getElementById('analysis-tabs');
    if (!tabs) return;
    tabs.querySelectorAll('a[data-mode]').forEach(a => {
...
function renderEntryAnalysis(stock, klines) {
    const result = analyzeEntry(klines, stock.current_price);

    if (result.error) {
        content.innerHTML = headerHtml(stock, '', null) + `<div class="alert alert-warning">${result.error}<br>
    ...
    let html = headerHtml(stock, `
        <span class="badge bg-${result.trend_class}">${result.trend}</span>
        <span class="badge bg-${badgeClass} ms-1">${result.entry_label}</span>`, null);
...
function renderAddAnalysis(stock, klines) {
    const result = analyzeAdd(klines, stock.current_price, stock.cost_price);

    if (result.error) {
        content.innerHTML = headerHtml(stock, '', 'add') + `<div class="alert alert-warning">${result.error}<br>
            <a href="stock-detail?id=${stock.id}" class="btn btn-sm btn-secondary mt-2">&larr; 返回</a></div>`;
        bindTabs();
        return;
    }

    const badgeClass = result.add_badge_class;
    const newsCardId = 'news-card-' + Date.now();

    let html = headerHtml(stock, `
        <span class="badge bg-${result.trend_class}">${result.trend}</span>
        <span class="badge bg-${badgeClass} ms-1">${result.add_label}</span>`, 'add');
        ...
        <div class="card-body">
            <div class="row g-3 text-center">
                ...
                <div class="col-md-3 col-6">
                    ...
                    <small class="${textClass(stock.current_price, stock.cost_price)}">${result.pl_pct > 0 ? '+' : ''}${result.pl_pct}%</small>
                </div>
                <div class="col-md-3 col-6">
                    <small class="text-muted">最近支撑</small>
                    <div class="fs-5 fw-bold text-success">${result.nearest_support ? formatPrice(result.nearest_support.price) : '<span class="text-muted">—</span>'}</div>
    ...
    html += `<div class="card mb-4 border-${badgeClass}">
        <div class="card-header bg-${badgeClass} text-white d-flex justify-content-between align-items-center">
            <strong><i class="bi bi-plus-circle"></i> 加仓综合评分</strong>
            <span class="fs-5 fw-bold">${result.add_score} 分 — ${result.add_label}</span>
        </div>
        <div class="card-body">
            <div class="row mb-3">`;
    for (const b of result.score_breakdown) {
    ...
    html += `<div class="card mb-4">
        ...
        <div class="card-body">
            <p class="mb-2"><span class="badge bg-info fs-6">${result.volume_trend}</span></p>`;
    if (result.volume_trend === '放量上涨') {
        html += '<p class="small text-success"><i class="bi bi-check-circle"></i> 放量上涨表明资金积极介入，加仓信号较好</p>';
    } else if (result.volume_trend === '缩量调整') {
        html += '<p class="small text-warning"><i class="bi bi-exclamation-circle"></i> 缩量调整，加仓需等待放量确认</p>';
    } else if (result.volume_trend === '量价背离') {
        html += '<p class="small text-danger"><i class="bi bi-exclamation-triangle"></i> 量价背离，不宜加仓，需警惕回调</p>';
    } else {
        html += '<p class="small text-muted">量能表现正常，无异常信号</p>';
    }
    html += '</div></div>';
    ...
    // Suggested Add Zones
    html += `<div class="card mb-4 border-success">
        <div class="card-header bg-success text-white"><strong><i class="bi bi-cart-plus"></i> 建议加仓区间</strong></div>
        <div class="card-body">
            <table class="table table-sm table-hover mb-0">
                <thead class="table-light"><tr><th>加仓价位</th><th class="text-end">价格</th><th>理由</th></tr></thead><tbody>`;
    for (let i = 0; i < result.suggested_zones.length; i++) {
        const z = result.suggested_zones[i];
        const isFirst = i === 0;
        const rowClass = isFirst ? 'table-success' : '';
    ...
    if (result.consolidation) {
        ...
        html += `<p>上沿: <span class="fw-bold text-danger">${formatPrice(c.upper)}</span> | 下沿: <span class="fw-bold text-success">${formatPrice(c.lower)}</span></p>
            <p class="text-muted small">振幅 ${c.range_pct}% | ATR ${formatPrice(c.atr)}</p>`;
        html += '<p class="small text-muted"><i class="bi bi-info-circle"></i> 盘整期间可在下沿附近加仓，等待突破</p>';
    } else {
        html += '<p class="text-muted">未检测到明显整理平台</p>';
    }
    html += '</div></div>';
    ...
    html += `<div class="text-muted small mt-2">加仓分析基于最近 ${result.data_days} 个交易日数据，仅供参考不构成投资建议</div>`;
...
function renderHoldingAnalysis(stock, klines, activeTab) {
    const result = analyze(stock.code, stock.current_price, stock.cost_price, klines);

    if (result.error) {
        content.innerHTML = headerHtml(stock, '', activeTab) + `<div class="alert alert-warning">${result.error}<br>
            <a href="stock-detail?id=${stock.id}" class="btn btn-sm btn-secondary mt-2">&larr; 返回</a></div>`;
        bindTabs();
        return;
    }

    let html = headerHtml(stock, `<span class="badge bg-${result.trend_class}">${result.trend}</span>`, activeTab);
...
function renderInsufficient() {
    ...
    const dayCount = Array.isArray(currentKlines) ? currentKlines.length : 0;
    const newsCardId = 'news-card-' + Date.now();

    content.innerHTML = headerHtml(stock, '', null) + `
...
function renderHolding() {
    if (currentStock.status === 'holding' && currentMode === 'add') {
        renderAddAnalysis(currentStock, currentKlines);
    } else {
        renderHoldingAnalysis(currentStock, currentKlines, currentStock.status === 'holding' ? currentMode : null);
    }
}
...
async function render() {
    ...
    try {
        ...
        currentSiblings = await getStocksByPortfolio(currentStock.portfolio_id).catch(() => []);

        // Fetch kline first (fast), load news in background later
        content.innerHTML = `获取 ${currentStock.name}(${currentStock.code}) K线数据<span class="spin ms-2"></span>`;
        currentKlines = await fetchKlineCached(currentStock.code, 60);

        if (!Array.isArray(currentKlines) || currentKlines.length < 14) {
            renderInsufficient();
            return;
        }

        // Watching stocks get an entry/buy analysis instead of position management
        if (currentStock.status === 'watching') {
            renderEntryAnalysis(currentStock, currentKlines);
            return;
        }
